...
export class TypeToSchemaConverter {
  ...
  static analyzeValue(value: any): RuntimeTypeInfo {
    ...
    if (Array.isArray(value)) {
      const elementType =
        value.length > 0
          ? this.analyzeValue(value[0])
          : { type: "any", isArray: false, isOptional: false, isRecord: false };
      return {
    ...
    if (typeof value === "object") {
      ...
      if (values.length > 0) {
        ...
        const allSameType = values.every(
          (v) => this.analyzeValue(v).type === firstValueType.type
        );
   ...
   */
  static generateSchemaWithHints<T>(
    typeHints?: Partial<Record<keyof T, string>>
  ): SchemaInterface {
