...
export class ASTBuilder {
  ...
  private static readonly VALID_DATA_TYPES = ['string', 'number', 'boolean'] as const;
  ...
  private static _validatePosition(position: number): void {
    if (position < 0 || !Number.isInteger(position)) {
      throw new Error(`Invalid position: ${position}. Position must be a non-negative integer.`);
    }
  }
  
  private static _validatePath(path: string[]): void {
    if (!Array.isArray(path) || path.length === 0) {
      throw new Error('Field access path must be a non-empty array of strings.');
    }
    
    for (const segment of path) {
      if (typeof segment !== 'string' || segment.trim() === '') {
        throw new Error(`Invalid path segment: ${segment}. All segments must be non-empty strings.`);
      }
    }
  }
  
  private static _validateDataType(dataType: string): asserts dataType is 'string' | 'number' | 'boolean' {
    if (!this.VALID_DATA_TYPES.includes(dataType as any)) {
      throw new Error(`Invalid data type: ${dataType}. Must be one of: ${this.VALID_DATA_TYPES.join(', ')}`);
    }
  }
  
  private static _validateLogicalOperator(operator: string): asserts operator is 'AND' | 'OR' {
    if (!this.VALID_LOGICAL_OPERATORS.includes(operator as any)) {
      throw new Error(`Invalid logical operator: ${operator}. Must be one of: ${this.VALID_LOGICAL_OPERATORS.join(', ')}`);
    }
  }
  
  private static _validateArraySize(elements: any[]): void {
    if (elements.length > this.MAX_ARRAY_SIZE) {
      throw new Error(`Array too large: ${elements.length} elements. Maximum allowed: ${this.MAX_ARRAY_SIZE}`);
    }
  }
  ...
  static createComparison(
    ...
    if (!left) {
      throw new Error('Comparison node requires a left operand (field access).');
    }
    ...
    if (args) {
      ...
      for (let i = 0; i < args.length; i++) {
        if (!args[i] || args[i].type !== ASTNodeType.LITERAL) {
          throw new Error(`Invalid argument at index ${i}: Method call arguments must be literal nodes.`);
        }
      }
    }
  ...
  static createLiteral(
    ...
    if (actualType !== dataType) {
      throw new Error(`Type mismatch: value type '${actualType}' does not match declared type '${dataType}'.`);
    }
    ...
    if (dataType === 'string' && typeof value === 'string' && value.length > 10000) {
      throw new Error('String literal too long. Maximum length: 10000 characters.');
    }
  ...
  static createConstant(
    ...
    if (value.length > 1000) {
      throw new Error('Constant value too long. Maximum length: 1000 characters.');
    }
  ...
  static createArray(
    ...
    for (let i = 0; i < elements.length; i++) {
      if (!elements[i] || elements[i].type !== ASTNodeType.LITERAL) {
        throw new Error(`Invalid element at index ${i}: Array elements must be literal nodes.`);
      }
    }
...
export class ASTWalker {
  ...
  private static _checkDepth(): void {
    if (this._currentDepth >= this.MAX_DEPTH) {
      throw new Error(`Maximum traversal depth exceeded: ${this.MAX_DEPTH}. Possible circular reference or deeply nested structure.`);
    }
  }
  ...
  static walk<T>(node: ASTNode, visitor: ASTVisitor<T>): T {
    ...
    try {
      ...
      const contextError = new Error(`Error walking AST at position ${node.position}: ${(error as Error).message}`);
...
export class ASTAnalyzer {
  ...
  static validateAST(node: ASTNode): string[] {
    ...
    const visitor: ASTVisitor<void> = {
      visitConditional: (n) => {
        if (!n.condition) {
          errors.push(`Conditional node at position ${n.position} missing condition`);
        }
        if (!n.thenValue) {
          errors.push(`Conditional node at position ${n.position} missing then value`);
        }
        ...
        try {
          ...
          errors.push(`Error validating conditional at position ${n.position}: ${(error as Error).message}`);
        }
      },
      
      visitLogicalExpression: (n) => {
        if (!n.left || !n.right) {
          errors.push(`Logical expression at position ${n.position} missing operands`);
        }
        if (!['AND', 'OR'].includes(n.operator)) {
          errors.push(`Invalid logical operator at position ${n.position}: ${n.operator}`);
        }
        ...
        try {
          ...
          errors.push(`Error validating logical expression at position ${n.position}: ${(error as Error).message}`);
        }
      },
      ...
      visitComparison: (n) => {
        ...
        try {
          ...
          errors.push(`Error validating comparison at position ${n.position}: ${(error as Error).message}`);
        }
      },
      ...
      visitMethodCall: (n) => {
        ...
        try {
          ...
          errors.push(`Error validating method call at position ${n.position}: ${(error as Error).message}`);
        }
      },
      ...
      visitFieldAccess: (n) => {
        if (!n.path || n.path.length === 0) {
          ...
          for (const segment of n.path) {
            if (typeof segment !== 'string' || segment.trim() === '') {
              errors.push(`Invalid path segment in field access at position ${n.position}: "${segment}"`);
            }
          }
        }
      },
      ...
      visitLiteral: (n) => {
        ...
        if (actualType !== n.dataType) {
          errors.push(`Type mismatch in literal at position ${n.position}: expected ${n.dataType}, got ${actualType}`);
        }
      },
      
      visitConstant: (n) => {
        if (!n.value || typeof n.value !== 'string') {
          errors.push(`Constant node at position ${n.position} missing or invalid value`);
        }
      },
      
      visitArray: (n) => {
        if (!n.elements || !Array.isArray(n.elements)) {
          errors.push(`Array node at position ${n.position} missing or invalid elements`);
        } else {
          for (let i = 0; i < n.elements.length; i++) {
            if (!n.elements[i] || n.elements[i].type !== ASTNodeType.LITERAL) {
              errors.push(`Invalid element at index ${i} in array at position ${n.position}`);
            }
          }
        }
      }
   ...
   */
  static getCacheStats(): { fieldCacheSize: number; complexityCacheSize: number } {
    return {
