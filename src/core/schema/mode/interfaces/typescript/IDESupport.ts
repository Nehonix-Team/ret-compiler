...
export class ConditionalIDESupport {
  ...
  getCompletions(
    ...
    switch (context.type) {
      ...
      case 'value':
        completions.push(...this.getValueCompletions(context.operator, context.fieldType));
        break;
  ...
  getHoverInfo(expression: string, position: number): HoverInfo | null {
    ...
    if (errors.length > 0) {
      const error = errors.find(e => 
        position >= e.position && position <= e.position + 10
      );
  ...
  getDiagnostics(expression: string): Diagnostic[] {
    ...
    errors.forEach(error => {
      diagnostics.push({
        ...
        severity: error.type === 'SYNTAX_ERROR' 
    ...
    if (ast) {
      ...
      if (hasNested) {
        diagnostics.push({
          ...
          message: 'Expression contains nested conditionals. Ensure proper testing.',
          source: 'fortify-conditional',
   ...
   * Get context at cursor position
   */
  private getContextAtPosition(expression: string, position: number): {
    type: string;
    fieldType?: string;
    operator?: string;
  } {
   ...
   */
  private getFieldCompletions(dataSchema?: Record<string, any>): CompletionItem[] {
    const completions: CompletionItem[] = [];
    ...
    const commonFields = ['id', 'name', 'email', 'age', 'role', 'status', 'type', 'level'];
  ...
  private getOperatorCompletions(fieldType?: string): CompletionItem[] {
    const operators = [
      ...
      { label: '!=', detail: 'Not equals', doc: 'Check if field does not equal value' },
      { label: '>', detail: 'Greater than', doc: 'Check if field is greater than value (numbers)' },
      { label: '>=', detail: 'Greater or equal', doc: 'Check if field is greater than or equal to value (numbers)' },
      { label: '<', detail: 'Less than', doc: 'Check if field is less than value (numbers)' },
      { label: '<=', detail: 'Less or equal', doc: 'Check if field is less than or equal to value (numbers)' },
      { label: '~', detail: 'Matches regex', doc: 'Check if field matches regular expression (strings)' }
    ];
  ...
  private getMethodCompletions(fieldType?: string): CompletionItem[] {
    const methods = [
      { label: 'in', detail: 'In array', doc: 'Check if field value is in array', snippet: 'in(${1:value1,value2})' },
      { label: 'exists', detail: 'Exists', doc: 'Check if field exists and is not null' },
      { label: 'contains', detail: 'Contains', doc: 'Check if field contains value', snippet: 'contains(${1:value})' },
      { label: 'startsWith', detail: 'Starts with', doc: 'Check if string field starts with value', snippet: 'startsWith(${1:prefix})' },
      { label: 'endsWith', detail: 'Ends with', doc: 'Check if string field ends with value', snippet: 'endsWith(${1:suffix})' },
      { label: 'between', detail: 'Between', doc: 'Check if number field is between two values', snippet: 'between(${1:min,max})' }
    ];
   ...
   */
  private getValueCompletions(operator?: string, fieldType?: string): CompletionItem[] {
    const completions: CompletionItem[] = [];
    ...
    const commonStrings = ['admin', 'user', 'guest', 'active', 'inactive', 'pending'];
