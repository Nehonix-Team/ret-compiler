...
export class InternalSyntaxParser {
    ...
    private static parseRevolutionarySyntax(cleanSyntax: string): any | null {
        ...
        const match = cleanSyntax.match(/^when\s+(.+?)\s+\*\?\s+(.+?)(?:\s*:\s*(.+))?$/);
     ...
     * @internal
     */
    private static parseParenthesesSyntax(syntax: string): { condition: string; thenSchema: string; elseSchema?: string } | null {
     ...
     * @internal
     */
    private static parseConditionPart(condition: string): ConditionalPattern | null {
        ...
        for (const op of operators) {
            if (condition.includes(op)) {
                ...
                if (op !== "~") { // Don't parse regex patterns
    ...
    private static convertToConditionalConfig(pattern: ConditionalPattern): any {
        ...
        switch (pattern.operator) {
            ...
            case "!=":
                ...
                break;
            case ">":
                conditionalThen = builder.when((val) => typeof val === "number" && val > pattern.value);
                break;
            case "<":
                conditionalThen = builder.when((val) => typeof val === "number" && val < pattern.value);
                break;
...
export const ConditionalSyntaxUtils = {
    ...
    isConditional: InternalSyntaxParser.isConditionalSyntax.bind(InternalSyntaxParser)
};
