...
console.log("🔍 Test String Regex Validation (Fixed)");
console.log("=======================================\n");
...
// Test 4: Simple regex with length constraints (separate)
console.log("\nTest 4: Length constraints (separate from regex)");
const LengthSchema = Interface({
  code: "string(3,10)",
});

const lengthTests = [
  { input: "ABC", expected: "PASS", description: "minimum length valid" },
  {
    input: "ABCDEFGHIJ",
    expected: "PASS",
    description: "maximum length valid",
  },
  { input: "AB", expected: "FAIL", description: "too short" },
  { input: "ABCDEFGHIJK", expected: "FAIL", description: "too long" },
];

for (const test of lengthTests) {
  const result = LengthSchema.safeParse({ code: test.input });
  const actual = result.success ? "PASS" : "FAIL";
  const isCorrect = actual === test.expected;
