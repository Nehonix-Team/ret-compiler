...
class FortifyDiagnosticsProvider {
    ...
    validateConstraintSyntax(constraints, type, range) {
        ...
        for (const param of params) {
            ...
            if (param && !/^\d*\.?\d*$/.test(param)) {
                diagnostics.push(new vscode.Diagnostic(range, `Invalid constraint: "${param}". Expected a number or decimal.`, vscode.DiagnosticSeverity.Error));
            }
        }
        return diagnostics;
    }
    ...
    validateConditionalOperators(condition, range) {
        ...
        const comparisonMatch = cleanCondition.match(/(\w+)\s*([!~=><]+|~|!~)\s*(.+)/);
        if (comparisonMatch) {
        ...
        if (cleanCondition.trim()) {
            diagnostics.push(new vscode.Diagnostic(range, `Invalid condition: "${cleanCondition}". Expected field=value, field.method(), or field~pattern.`, vscode.DiagnosticSeverity.Warning));
        }
        return diagnostics;
    }
}
