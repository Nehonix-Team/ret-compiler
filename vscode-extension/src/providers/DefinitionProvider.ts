...
export class FortifyDefinitionProvider implements vscode.DefinitionProvider {
   ...
   */
  private findInterfaceBlocks(lines: string[]): Array<{ start: number; end: number }> {
   ...
   */
  private isVariableInConditional(lineText: string, word: string, charPosition: number): boolean {
  ...
  private findPropertyDefinition(
    ...
    const interfaceBlocks = this.findInterfaceBlocks(lines);
    const currentBlock = interfaceBlocks.find(block => 
